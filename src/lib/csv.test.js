...
describe('stagesToCSV', () => {
	it('generates correct CSV with transposed layout (rows=metrics, cols=days)', () => {
		...
		expect(lines[0]).toBe(',Day 1,Day 2,Total');
		expect(lines[1]).toBe('Distance (km),12.34,8.5,20.84');
		expect(lines[2]).toBe('Up (m),456,200,656');
		expect(lines[3]).toBe('Down (m),320,150,470');
		expect(lines[4]).toBe('Performance Distance (Lkm),16.9,10.5,27.4');
	...
	it('handles empty stages array', () => {
		...
		expect(lines[1]).toBe('Distance (km),0');
		expect(lines[2]).toBe('Up (m),0');
		expect(lines[3]).toBe('Down (m),0');
		expect(lines[4]).toBe('Performance Distance (Lkm),0');
	});
});
