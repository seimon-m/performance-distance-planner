...
- **Route map** — interactive 3D satellite map with color-coded route (elevation or steepness), waypoint markers that follow the selected tent spots (alternatives shown as labeled hollow dots), and terrain toggle; the MapLibre GL bundle is dynamically imported only when the map is revealed, with a static placeholder shown until then
